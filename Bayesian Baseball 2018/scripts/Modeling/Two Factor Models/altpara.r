...
mdiff <- map2stan(
  alist(
    ...
    B <- hits_h[home] * Hh + double_h[home] * B2Bh + triple_h[home] * B3Bh + 
      ...
      hits_allowed_h[home] * HHAh + pballs_h[home] * PBBh + 
      pstrikeouts_h[home] * PSOh +  strikes_h[home] * Strh + b,
    #away
    C <- hits_a[away] * Ha + double_a[away] * B2Ba + triple_a[away] * B3Ba + 
      HR_a[away] * BHRa + balls_a[away] * BBBa +       
    ...
    sigma ~ dcauchy(0,2)),
  data=mydata,
  iter=2000, warmup=250, chains=1, cores=4, control = list(adapt_delta = 0.99, max_treedepth = 15))
